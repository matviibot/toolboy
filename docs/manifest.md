...
Status: **design draft**. One file at the repo root. It's the contract every layer
reads: the client loader, the backend discovery index, and the trust UI. A repo's
branch/HEAD is the **mutable pointer**; a specific commit is the **immutable pin**
(see [loading.md](loading.md)).
