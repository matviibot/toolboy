...
|---|---|
| `storage` | Namespaced IndexedDB, keyed by the array key `[repo, toolId, key]` — a structured key, not a `"<toolId>::<key>"` string, so there is no delimiter to inject or collide on. Denied entirely if `permissions.storage` is false. A tool only ever sees its own keys. Durability is a host concern the tool never sees: [persistence.md](persistence.md). |
...
| `net.fetch` | Host checks the URL host against the tool's domain allowlist, **injects declared secrets** into request headers (host-side, from the keyring), performs the real fetch, returns a serialized response. A direct fetch is tried first; if it fails on CORS/network and a backend is configured (`VITE_BACKEND_URL`), the assembled request falls back to the relay at `<backend>/relay` ([backend/](../backend/)). Redirects are never followed on either path. |
